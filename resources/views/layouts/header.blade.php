    ...
    <nav class="navbar navbar-expand-lg theme-navbar">
      ...
      <span class="navbar-toggler-icon"></span>
            ...
            <div>
                ...
                <a href="{{url('vendor/signup')}}" target="blank" class="text-decoration-none text-light">Become a Seller</a>
                ...
                <a href="{{url('user/' . $user->user_id)}}" class="btn theme-orange-btn btn-sm text-light ms-1 rounded-pill px-3 py-2"><i class="fa-solid fa-user"></i> Dashboard</a>
                @else
            ...
            <div class="collapse navbar-collapse justify-content-center" id="navbarNav">
                <ul class="nav">
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle text-dark" href="{{url('category/electronics')}}" role="button" data-bs-toggle="dropdown" aria-expanded="false"> {{$cat->c_name}} </a>

                        @if ($cat->subCategory->count()>0)
                        <ul class="dropdown-menu">
                            <li><a class="dropdown-item" href="{{route('category', $cat->c_name)}}">All</a></li>
                            @foreach ($cat->subCategory as $subcat)
                            <li><a class="dropdown-item" href="{{route('category.sub_category', ['category'=>$cat->c_name, 'sub_category'=>$subcat->c_name])}}">{{ $subcat->c_name}}</a></li>
                            @endforeach
                        </ul>
                        @endif
                    </li>
